<div>
    <button class="btn rounded-full border border-primary text-primary hover:bg-primary hover:text-white"
        data-fc-target="modalDocument-{{ $person->id }}" data-fc-type="modal" type="button">
        Verificar RFC
    </button>
    <div id="modalDocument-{{ $person->id }}"
        ...
        <div
            ...
            <div class="px-4 py-8 flex">
                <form action="{{ route('admin.client.uploadDocument', $person->id) }}" method="post"
            ...
            <div class="flex justify-end items-center gap-4 p-4 border-t ">
                ...
                @livewire('client.upload-cif', ['person' => $person])
            </div>
        </div>
    </div>
</div>
