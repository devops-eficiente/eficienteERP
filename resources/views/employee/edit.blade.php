...
@section('content')
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="md:flex hidden items-center gap-2.5 text-sm font-semibold">
            ...
            <div class="flex items-center gap-2">
                <i class="mgc_right_line text-lg flex-shrink-0 text-slate-400 rtl:rotate-180"></i>
                <a href="#" class="text-sm font-medium text-slate-700 dark:text-slate-400" aria-current="page">Editar
                    Empleados</a>
            </div>
        </div>
    </div>
    <form action="{{ route('admin.update_employee', $person->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        {{-- @if ($errors->any())
            <div class="my-2">
        ...
        <div class="card">
            ...
            <div class="p-6">
                ...
                <div class="grid lg:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input type="text" id="simpleinput" class="form-input" name="paternal_surname"
                            value="{{ old('paternal_surname', $person->employee->paternal_surname) }}">
                    ...
                    <div>
                        ...
                        <input type="text" id="simpleinput" class="form-input" name="maternal_surname"
                            value="{{ old('maternal_surname', $person->employee->maternal_surname) }}">
                    ...
                    <div>
                        ...
                        <input type="text" id="simpleinput" class="form-input" name="name"
                            value="{{ old('name', $person->employee->name) }}">
                        @error('name')
                            <span class="text-red-800">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="grid lg:grid-cols-4 gap-6 my-4">
                    <div>
                        ...
                        <input type="text" id="simpleinput" class="form-input" name="curp"
                            value="{{ old('curp', $person->employee->curp) }}">
                        @error('curp')
                            <span class="text-red-800">{{ $message }}</span>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="text" id="rfc_input" class="form-input" name="rfc"
                            value="{{ old('rfc', $person->rfc) }}" oninput="validarInput(this)">
                    ...
                    <div>
                        ...
                        <input class="form-input" id="example-date" type="date" name="birthdate"
                            value="{{ old('birthdate', $person->employee->birthdate) }}">
                        @error('birthdate')
                            <span class="text-red-800">{{ $message }}</span>
                        @enderror
                    </div>
                        ...
                        <select class="form-select" id="example-select" name="tax_regime_id">
                            ...
                            @foreach ($taxRegimes as $taxRegime)
                                <option value="{{ $taxRegime->id }}"
                                    {{ $person->tax_regimes->contains($taxRegime) ? 'selected' : '' }}>
                ...
                <div class="grid lg:grid-cols-2 gap-6 my-4">
                        ...
                        <select class="form-select" id="example-select" name="institute_health_id">
                            ...
                            @foreach ($instituteHealths as $instituteHealth)
                                <option value="{{ $instituteHealth->id }}"
                                    {{ $person->employee->institute_health_id == $instituteHealth->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="text" id="simpleinput" class="form-input" name="nss"
                            value="{{ old('nss', $person->employee->nss) }}">
                        @error('nss')
                            <span class="text-red-800">{{ $message }}</span>
                        @enderror
                    </div>
                        ...
                        <select class="form-select" id="example-select" name="identification_employee_id">
                            ...
                            @foreach ($identificationEmployees as $identificationEmployee)
                                <option value="{{ $identificationEmployee->id }}"
                                    {{ old('identification_employee_id', $person->employee->identification_employee_id) == $identificationEmployee->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="text" id="simpleinput" class="form-input" name="n_identification"
                            value="{{ old('n_identification', $person->employee->n_identification) }}">
                ...
                <div class="grid lg:grid-cols-4 gap-6 my-4">
                        ...
                        <select class="form-select" id="example-select" name="marital_status_id">
                            ...
                            @foreach ($maritalStatus as $marital)
                                <option value="{{ $marital->id }}"
                                    {{ old('marital_status_id', $person->employee->marital_status_id) == $marital->id ? 'selected' : '' }}>
                                    {{ $marital->name }}</option>
                            @endforeach
                        </select>
                        ...
                        <select class="form-select" id="example-select" name="blood_type_id">
                            ...
                            @foreach ($bloodTypes as $bloodType)
                                <option value="{{ $bloodType->id }}"
                                    {{ old('blood_type_id', $person->employee->blood_type_id) == $bloodType->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select class="form-select" id="example-select" name="nationality">
                            ...
                            <option value="Mexicana"
                                {{ old('nationality', $person->employee->nationality) == 'Mexicana' ? 'selected' : '' }}>
                                Mexicana</option>
                    ...
                    <div>
                        <label for="example-select" class="text-gray-800 text-sm font-medium inline-block mb-2">
                            Genero {{ $person->employee->gender }}
                        </label>
                        <select class="form-select" id="example-select" name="gender">
                            {{-- <option value="{{ $person->employee->gender }}">{{ $person->employee->gender }}</option> --}}
                            <option value="">Selecciona una categoria</option>
                            <option value="Hombre"
                                {{ old('gender', $person->employee->gender) == 'Hombre' ? 'selected' : '' }}>
                                Hombre
                            </option>
                            <option value="Mujer"
                                {{ old('gender', $person->employee->gender) == 'Mujer' ? 'selected' : '' }}>
                                Mujer
                            </option>
                            <option value="Otro"
                                {{ old('gender', $person->employee->gender) == 'Otro' ? 'selected' : '' }}>
                                Otro
                            </option>
                        </select>
                        @error('gender')
                ...
                <div class="grid lg:grid-cols-3 gap-6 my-4">
                    <div>
                        ...
                        <div class="relative">
                            <input type="email" id="leading-icon" name="contacts[email]"
                                value="{{ old('contacts[email]', $contact->email) }}" class="form-input ps-11"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="text" id="leading-icon" name="contacts[telephone]"
                                value="{{ old('contacts[telephone]', $contact->phone) }}" class="form-input ps-11"
                                placeholder="#########">
                            <div class="absolute inset-y-0 start-4 flex items-center z-20">
                    ...
                    <div>
                        ...
                        <input type="text" id="simpleinput" class="form-input" name="zip_code"
                            value="{{ old('zip_code', $person->addresses[0]->zip_code) }}">
                        @error('zip_code')
                            <span class="text-red-800">{{ $message }}</span>
                        @enderror
                    </div>
        ...
        {{-- <div class="card mt-6">
            ...
            <div class="p-6">
                @php
                    $emergencyContacts = $person->employee->emergency_contacts;
                @endphp
                <div class="grid lg:grid-cols-2 gap-6 my-4">
                    <div>
                        <label for="simpleinput" class="text-gray-800 text-sm font-medium inline-block mb-2">
                        ...
                        <div class="relative">
                            <input type="text" id="leading-icon" name="emergency_contacts[name]"
                                value="{{ old('emergency_contacts[name]',$emergencyContacts['name']) }}" class="form-input ps-11"
        ...
        <div class="flex items-center justify-around my-4">
            <button type="submit" class="btn bg-success text-white rounded-full">Guardar</button>
            <a href="{{ route('admin.employees') }}" class="btn bg-warning text-white rounded-full">Cancelar</a>
        </div>
    </form>
