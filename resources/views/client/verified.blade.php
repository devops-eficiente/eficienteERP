...
@section('content')
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 overscroll-y-auto">
                            ...
                            <tbody class="divide-y divide-gray-200">
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                                        @if (strcasecmp($person->client->company_name, $persona->razon_social) !== 0)
                                            <i class="mgc_close_fill text-lg text-green-700"></i>
                                            @php
                                                $cont++;
                                            @endphp
                                ...
                                @if ($person->client->capital_regime_id)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 ">
                                            {{ $persona->regimen_de_capital }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                                            @php
                                                $status = false;
                                                $regimenSat = str_replace(' ', '', $person->regimen_de_capital);
                                                $regimenPersona = str_replace(' ', '', $person->client->capital_regime->acronym);
                                                if ($regimenSat == $regimenPersona) {
                                                    $status = true;
                                                }
                                            @endphp
                                            @if ($status)
    ...
    <div class="flex justify-around my-3">
        ...
        @if ($cont == 0)
            <a href="{{ route('admin.client.continue_employee') }}" class="btn bg-success text-white">Continuar</a>
        @else
            <a href="{{ route('admin.client.edit_data_employee') }}" class="btn bg-warning text-white">Corregir
                errores</a>
        @endif
    </div>
@endsection
